...
fun Stock.updatePrice(priceDto: StockPriceDto) {
    ...
    this.dayDelta = delta
    this.dayDeltaPercent =
        if (priceDto.previousClose != 0.0f) abs(delta * 100.0f) / priceDto.previousClose else 0.0f
}

fun RatiosDto.toRatios(): List<Ratio> {
    val ratios = mutableListOf<Ratio>()
    currentRatio?.let {ratios.add( Ratio(name = "currentRatio", value = it))}
    peRatio?.let {ratios.add( Ratio(name = "peRatio", value = it))}
    priceToSalesRatio?.let {ratios.add( Ratio(name = "priceToSalesRatio", value =  it))}
    priceBookValueRatio?.let {ratios.add( Ratio(name = "priceBookValueRatio", value =  it))}
